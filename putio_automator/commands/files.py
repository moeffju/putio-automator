...
from putio_automator import logger
    ...
    if len(files):
        def func(connection):
            ...
            for current_file in files:
                conn.execute("SELECT DATETIME(created_at, 'localtime') FROM downloads WHERE name = ? AND size = ?",
                ...
                if row is None or force:
                    ...
                    current_file.download(
                        dest=ctx.obj['INCOMPLETE'], delete_after_download=True, chunk_size=int(chunk_size)*1024)
                    logger.info('downloaded file: %s' % current_file.name)

                    src = os.path.join(
                        ctx.obj['INCOMPLETE'], current_file.name)
                    dest = os.path.join(ctx.obj['DOWNLOADS'], folder)
                    shutil.move(src, dest)

                    conn.execute('INSERT INTO downloads (id, name, size) VALUES (?, ?, ?)',
                                 (current_file.id, current_file.name, current_file.size))
                    connection.commit()
                    ...
                    logger.warning('file already downloaded at %s : %s' % (
                        row[0], current_file.name))
